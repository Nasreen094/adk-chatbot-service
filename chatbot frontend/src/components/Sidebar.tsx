...
import { addChat, useAppDispatch } from "@/redux";
...
export const Sidebar = () => {
	...
	const chatHistory = useAppSelector((state) => state.chats.chats);
	...
	return (
		<ShadcnSidebar className="">
			<div className="h-full bg-[#E2E8F0] mt-[80px]">
				<SidebarHeader className="h-[60px] flex flex-row flex-1 items-center px-4 gap-2">
					<SidebarTrigger />
				...
				<SidebarContent className="flex flex-col flex-1 h-full">
										...
										<Link href={`/c/${chat.id}`}>
											<Button
												...
												className={cn(
													...
													{
														"navbar-gradient text-white hover:text-white":
											...
											>
												...
												<span>Chat {chatHistory.length - index}</span>
											</Button>
										</Link>
									</li>
								))}
