...
interface ChartProps {
	...
	dataTabs?: { label: string; value: string }[];
	activeTab?: string;
...
const AppChartComponent: React.FC<ChartProps> = ({
	...
	useEffect(() => {
		if (isModalOpen) {
			// Calculate modal height as 80% of window height minus padding (p-4 = 16px * 2)
			setModalHeight(0.8 * window.innerHeight - 32);
		}
	}, [isModalOpen]);
	...
	const modalContent = useMemo(() => {
		...
		return (
			...
			>
				<div className="relative flex h-[90vh] w-4/5 flex-col overflow-y-auto bg-white p-4">
					<button
						className="absolute right-4 top-4 z-[10000] text-gray-500 hover:text-gray-700"
	...
	return (
		...
		>
			<>
				...
				{showExpandIcon && (
					<div className="relative bottom-5 flex w-full items-center justify-end">
						<button
